...
from pages.assistant.code_review import CodeReview
from pages.assistant.translation import Translation
from share.github.client.github_client import GitHubClient
...
def code_review(code: str) -> str:
    ...
    """
    try:
        return CodeReview.code_review(code)
    except Exception as e:
        return f"코드 리뷰 실패: {str(e)}"


@tool
def get_github_pr(url: str) -> str:
    """
    github pull 주소를 받아 github api로 변경사항을 조회하는 도구

    :param url: pull request 주소
    :return:
        리뷰 대상 수정 코드 리스트
    """
    try:
        return GitHubClient.get_pr_diff(GitHubClient.replace_pr_api_url(url))
    except Exception as e:
